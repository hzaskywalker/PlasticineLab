...
def plot_data(data, xaxis='Epoch', value="AverageEpRet", condition="Condition1", smooth=1, **kwargs):
    ...
    plt.legend(loc='upper center', ncol=1, handlelength=1,
               borderaxespad=0., bbox_to_anchor=(0.5, 1.3), prop={'size': 10})
        ...
        if 'progress.txt' in files:
            ...
            try:
                exp_data = pd.read_table(
                    os.path.join(root, 'progress.txt'), sep=',')
            except:
...
def make_plots(output, all_logdirs, legend=None, xaxis=None, values=None, count=False,
...
def main():
    ...
    parser.add_argument('--value', '-y', default='last_iou', nargs='*')
    parser.add_argument('--count', action='store_true')
    parser.add_argument('--smooth', '-s', type=int, default=1)
    parser.add_argument('--select', nargs='*')
    parser.add_argument('--exclude', nargs='*')
    parser.add_argument('--est', default='mean')
    parser.add_argument('--output', default='bench.png')
    ...
    """
    print(args.output)
    make_plots(args.output, args.logdir, args.legend, args.xaxis, args.value, args.count,
